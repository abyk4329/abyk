...
function calculateNameValue(fullName?: string): {
  raw: number;
  reduced: number;
} {
...
function calculateBirthValue(birthDate?: string): {
  raw: number;
  reduced: number;
} {
...
export function calculateWealthCode(
  ...
  if (input.fullName) {
    breakdown.push(
      `ערך השם (${input.fullName}): ${name.reduced} (סכום גולמי ${name.raw})`
    );
  }
  if (input.birthDate) {
    breakdown.push(
      `ערך תאריך הלידה (${input.birthDate}): ${birth.reduced} (סכום גולמי ${birth.raw})`
    );
  }
 ...
 * Provide your official texts in: features/wealth-code/data/digitInterpretations.ts
...
export function getAllInterpretations(): Array<
  Interpretation & { code: number }
> {
  return [];
}
