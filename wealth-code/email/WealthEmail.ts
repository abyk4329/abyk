...
const INLINE_CONTENT_STYLES = {
  ...
  infoCard: `background:${
    EMAIL_GRADIENTS.panelHighlight
  };border-radius:24px;padding:20px 24px;margin:0 auto 32px;max-width:520px;box-shadow:${
    EMAIL_SHADOWS.infoCard
  };border:1px solid ${withOpacity(
    EMAIL_PALETTE.shadowHighlight,
    0.5
  )};text-align:center;direction:rtl;`,
  infoText: `font-family:'Assistant', Arial, sans-serif;font-size:16px;font-weight:600;line-height:1.6;color:${EMAIL_PALETTE.accent};margin:0;`,
};
...
function generateWealthContent(data: WealthEmailData): string {
  ...
  const pdfDownloadUrl = `${siteBase}/api/generate-pdf?code=${code}`;
  ...
  return `
    ...
    <p class="message" style="${INLINE_CONTENT_STYLES.message}">
      צרפנו למייל את הפירוש המלא בקובץ PDF להורדה מיידית, ותמיד אפשר לחזור לאתר כדי לצפות בו און־ליין מכל מכשיר.
    </p>

    <div class="info-card" style="${INLINE_CONTENT_STYLES.infoCard}">
      <p class="info-text" style="${INLINE_CONTENT_STYLES.infoText}">הקובץ כולל את כל הפירושים, מוקדי הצמיחה והיישום היומי בקובץ אחד. מומלץ לשמור אותו במחשב או בענן להמשך.</p>
      <p class="info-text" style="${INLINE_CONTENT_STYLES.infoText}margin-top:12px;">אם הקובץ לא נפתח בלחיצה, הורידו אותו באמצעות הכפתור המצורף או שמרו את הקובץ ישירות מהודעות המייל.</p>
    </div>

    <div class="buttons-container" style="${INLINE_CONTENT_STYLES.buttonsContainer}">
        <a href="${pdfDownloadUrl}" style="${PRIMARY_BTN_STYLE}" target="_blank" rel="noopener noreferrer">הורדת קובץ ה-PDF</a>

        <a href="${interpretationsUrl}" style="${SECONDARY_BTN_STYLE}" target="_blank" rel="noopener noreferrer">צפייה מלאה באתר</a>
