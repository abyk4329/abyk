...
export function WealthCodeSalesPage({
  ...
  useEffect(() => {
    try {
      ...
      console.warn('Unable to persist lastWealthCode during effect:', storageError);
    }
  }, [wealthCode]);
    ...
    try {
      if (wealthCode && Number.isFinite(wealthCode)) {
        try {
          ...
          console.warn('Unable to set lastWealthCode in localStorage:', storageError);
        }
        try {
          sessionStorage.setItem('lastWealthCode', String(wealthCode));
        } catch (storageError) {
          console.warn('Unable to set lastWealthCode in sessionStorage:', storageError);
        }
      }
    } catch (error) {
      console.warn('Persisting lastWealthCode before purchase failed:', error);
    }
    ...
    try {
      if (wealthCode && Number.isFinite(wealthCode)) {
        try {
          ...
          console.warn('Unable to set lastWealthCode in localStorage for simulation:', storageError);
        }
        try {
          sessionStorage.setItem('lastWealthCode', String(wealthCode));
        } catch (storageError) {
          console.warn('Unable to set lastWealthCode in sessionStorage for simulation:', storageError);
        }
      }
    } catch (error) {
      console.warn('Persisting lastWealthCode before simulation failed:', error);
    }
