import type { TextareaHTMLAttributes } from 'react';

interface NeuTextareaProps extends TextareaHTMLAttributes<HTMLTextAreaElement> {
  label: string;
}

export default function NeuTextarea({ label, ...props }: NeuTextareaProps) {
  return (
    <div className="relative flex flex-col gap-2">
      <textarea
        {...props}
        className="w-full min-h-[100px] px-4 py-3 rounded-lg text-base bg-transparent text-text transition-all duration-300 neu-inset-min focus:shadow-neu-inset focus:outline-none peer resize-y"
      />
      <label
        className="absolute right-4 top-3 text-sm text-text/70 pointer-events-none transition-all duration-300 peer-focus:text-xs peer-focus:-translate-y-7 peer-focus:text-accent peer-valid:text-xs peer-valid:-translate-y-7"
        dir="rtl"
      >
        {label}
      </label>
    </div>
  );
}
