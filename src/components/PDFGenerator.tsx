...
export class WealthCodePDFGenerator {
  ...
  generatePDF(wealthCode: number, codeStructure: CodeStructure, digitData: WealthCodeData[]): Uint8Array {
    ...
    // Pattern explanation
    // Structure summary (kept concise to fit PDF)
    let patternText = '';
    if (codeStructure.allSame) {
      patternText = 'קוד מאסטר - כל הספרות זהות. עוצמה מרוכזת, מתנה של כוח ממוקד לצד צורך באיזון.';
    } else if (codeStructure.hasRepeats) {
      patternText = 'ספרות חוזרות בקוד. אנרגיות מועצמות שמחייבות תשומת לב ואיזון בין תחומי החיים.';
    } else if (codeStructure.allDifferent) {
      patternText = 'קוד מגוון - כל הספרות שונות. מגוון עשיר של אנרגיות הדורש הרמוניה פנימית.';
    }
