...
const extractCodeFromCandidate = (candidate: unknown): number | undefined => {
  ...
  if (typeof candidate === 'string') {
    ...
    const hintMatch =
      /(?:^|[^a-zA-Z])(code|wealth)(?:[^a-zA-Z]|$)/i.test(trimmed) ||
      /(?:^|\s|[^א-ת])קוד(?:\s|[^א-ת]|$)/u.test(trimmed)
  ...
  try {
    ...
    if (email && successStatuses.includes(status) && wealthCode) {
      try {
        ...
        const html = generateEmailHTML({ wealthCode, viewUrl, downloadUrl })
        const text = generateEmailText({ wealthCode, viewUrl, downloadUrl })
