...
import { wealthEmailHtml, buildWealthEmailSubject } from "@/modules/wealth-code/email/template";
...
export function validateWealthEmailRequest(body: WealthEmailRequest) {
    ...
    if (!code || !/^\d{4}$/.test(code)) {
        throw new WealthEmailError('Invalid or missing "code" - must be exactly 4 digits');
    }

    return { intendedRecipient, code };
}

export async function dispatchWealthEmail(body: WealthEmailRequest, request: Request | null = null): Promise<WealthEmailResponse> {
