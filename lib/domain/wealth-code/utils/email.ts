...
export async function sendWealthEmail(
  params: SendWealthEmailParams
): Promise<SendWealthEmailSuccess> {
  const cleanedBody =
    params.body?.filter((line) => Boolean(line?.trim())) ?? [];
  ...
  const pdfBase64 =
    typeof pdfJson.pdfBase64 === "string"
      ? stripBase64Prefix(pdfJson.pdfBase64)
      : "";
