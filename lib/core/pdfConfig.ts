...
export function registerHebrewFonts(opts: HebrewFontOptions = {}) {
  ...
  if (foundAny) {
    try {
      ...
      console.log(
        `[pdfConfig] Registered Assistant font family with ${fontConfig.fonts.length} weights (200-800)`
      );
      return;
    } catch (e) {
  ...
  console.warn(
    "[pdfConfig] Assistant fonts not found in public/fonts/Assistant /. Hebrew text may not render properly."
  );
}
