export default function Header() {
  return (
    <header className="fixed inset-x-0 top-0 z-40 w-full border-b border-gold-primary/30 bg-ivory/90 backdrop-blur-md">
      <div className="px-4 py-3 md:py-4">
        <div className="assistant-light uppercase text-center tracking-[0.15em] text-espresso">
          <span className="block text-[var(--ms--1)] md:text-[var(--ms-0)] lg:text-[var(--ms-1)]">
            YOUR PERSONAL SPACE FOR GROWTH
          </span>
        </div>
      </div>
      <div className="h-[2px] bg-gradient-to-r from-transparent via-gold-primary/50 to-transparent" />
    </header>
  );
}
