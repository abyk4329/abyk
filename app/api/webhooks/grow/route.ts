  ...
  for (const field of fields) {
    if (field && typeof field === "object") {
      const value =
        ...
        (field as Record<string, unknown>).answer;
      const key =
...
function shouldProcessEvent(payload: any): {
  proceed: boolean;
  reason?: string;
} {
  const eventType =
  ...
  const statusValueRaw =
    payload?.data?.status ?? payload?.status ?? payload?.order?.status;
  const statusValue =
    typeof statusValueRaw === "string"
      ? statusValueRaw.toLowerCase()
      : undefined;
...
export async function POST(req: Request) {
  try {
    ...
    if (!code) {
      return NextResponse.json(
        { ok: false, error: "Missing wealth code in webhook payload" },
        { status: 400 }
      );
    }
    ...
    if (!email) {
      return NextResponse.json(
        { ok: false, error: "Missing customer email in webhook payload" },
        { status: 400 }
      );
    }
