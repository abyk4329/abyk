...
function extractPayload(pre: any) {
  ...
  const legacyStatus = (pre?.paymentStatus || pre?.status || "")
    .toString()
    .toLowerCase();
  ...
  const statusCode = Number.parseInt(
    String(data?.statusCode ?? data?.status_code ?? "NaN"),
  );
  ...
  const dynamicFields: Dyn[] = Array.isArray(data?.dynamicFields)
    ? data.dynamicFields
    : [];
  const pickVal = (keys: string[]): number | 0 => {
    ...
    for (const k of keys) {
      const v = Number.parseInt(
        String(legacyCustom[k as keyof typeof legacyCustom] ?? ""),
      );
      if (Number.isFinite(v)) return v as number;
    }
    return 0;
  ...
  const isSuccess =
    ...
    successStatuses.has(legacyStatus) ||
    statusCode === 2;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const queryKey =
      url.searchParams.get("webhook_key") || url.searchParams.get("key") || "";
    const bodyKey = (pre?.webhook_key || pre?.key || "").toString();
    const apiKeyOk = [headerApiKey, queryKey, bodyKey].some(
      (k) => k && k.trim() === secret,
    );
    ...
    if (!email) {
      return NextResponse.json(
        { message: "Missing customerEmail" },
        { status: 400 },
      );
    }
    if (
      !isValidDigit(code.bd) ||
      !isValidDigit(code.bm) ||
      !isValidDigit(code.by) ||
      !isValidDigit(code.lp)
    ) {
      return NextResponse.json(
        { message: "Invalid or missing code values" },
        { status: 400 },
      );
    }
    ...
    const hasDuplicates =
      numbers.length !== Array.from(new Set(numbers)).length;
    const baseUrl = (
    ...
    if (!transporter) {
      return NextResponse.json({
        message: "Email disabled (no SMTP credentials)",
        interpretationUrl,
      });
    }
