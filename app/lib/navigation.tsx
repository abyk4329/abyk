...
const NavigationOverridesContext = createContext<NavigationOverrides | undefined>(undefined);
const NavigationSetterContext = createContext<NavigationSetter | undefined>(undefined);
...
export function NavigationRoot({ children }: NavigationRootProps) {
  const [overrides, setOverrides] = useState<NavigationOverrides | undefined>(undefined);
...
export function useNavigationOverrides(): NavigationOverrides {
  ...
  if (!context) {
    throw new Error("useNavigationOverrides must be used within a NavigationRoot");
  }
